...
export class ProductDetailComponent implements OnInit {
      ...
      next: (res) => {
        if (res.success) {
          ...
          if (this.product.description) {
            this.features = this.product.description
              .split(',')
              .map((f: string) => f.trim());
          }
  ...
  get filteredReviews() {
    ...
    return this.reviews.filter(
      r => r.rating === this.selectedFilter
    );
  }
  ...
  extractSpecs() {
    ...
    if (ramMatch) {
      this.specs['RAM'] = ramMatch[1];
    }

    // Storage detect
    const storageMatch = desc.match(/(\d+gb|\d+tb)/);
    if (storageMatch) {
      this.specs['Storage'] = storageMatch[1];
    }

    // Processor detect
    if (desc.includes('m3')) {
      this.specs['Processor'] = 'Apple M3';
    }

    if (desc.includes('a17')) {
      this.specs['Processor'] = 'A17 Pro';
    }

    if (desc.includes('i7')) {
      this.specs['Processor'] = 'Intel i7';
    }
  }
